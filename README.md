...
And enable the plugin by adding ```dobo```m ```dobo-extra``` and ```bajo-extra``` to the plugin list in either the ```$dataDir/config/.plugins``` file or the ```bajo.plugins``` array within your ```package.json``` file
